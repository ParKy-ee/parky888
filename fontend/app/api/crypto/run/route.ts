...
const projectRoot = path.join(process.cwd(), "..", "cryto");
const runnerPath = path.join(projectRoot, "models_", "run_all_models.py");
const historyPath = path.join(projectRoot, "logs", "crypto_run_history.jsonl");
...
export async function POST() {
  if (running) {
    return NextResponse.json(
      {
        error: "Crypto models are already running"
      },
      { status: 409 }
    );
  }
  ...
  try {
    ...
    if (!result) {
      throw lastError ?? new Error("Failed to execute crypto model runner.");
    }
