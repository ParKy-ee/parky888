...
MODEL_VERSION = "AUDUSD-forecast-v0.20-risk-commodity-momentum"
...
def add_htf(df, interval):
    ...
    if len(htf) < 80:
        ...
        return df
    fast, slow = (10, 50) if rule == "1d" else (10, 200)
...
def score_row(row):
    score, reasons = 0, []
    if row["close"] > row["ema200"] and row["ema10"] > row["ema50"]:
        score += 3; reasons.append("risk_trend_bull")
    elif row["close"] < row["ema200"] and row["ema10"] < row["ema50"]:
        score -= 3; reasons.append("risk_trend_bear")
    if row["macd_hist"] > 0 and row["roc"] > 0:
        score += 1; reasons.append("momentum_up")
    elif row["macd_hist"] < 0 and row["roc"] < 0:
        score -= 1; reasons.append("momentum_down")
    if row["cci"] > 100:
        score += 1; reasons.append("cci_bull_expansion")
    elif row["cci"] < -100:
        score -= 1; reasons.append("cci_bear_expansion")
    if row["atr_ratio"] >= 1.05 and row["adx"] >= 20:
        score += 1 if score > 0 else -1 if score < 0 else 0; reasons.append("risk_vol_confirm")
    if row["rsi"] >= 72:
        score -= 1; reasons.append("overbought_risk")
    elif row["rsi"] <= 28:
        score += 1; reasons.append("oversold_risk")
...
def forecast_interval(interval):
    ...
    action = "WAIT" if bias == "NEUTRAL" else f"{bias}_RISK_MOMENTUM_WATCH"
    return {
        ...
        "confidence": min(abs(score) / 9, 1.0), "score": score,
        ...
        "stop_loss": row["close"] - 1.25 * row["atr"] if bias == "BULLISH" else row["close"] + 1.25 * row["atr"] if bias == "BEARISH" else np.nan,
        "take_profit_1": row["close"] + 1.8 * row["atr"] if bias == "BULLISH" else row["close"] - 1.8 * row["atr"] if bias == "BEARISH" else np.nan,
...
def main():
    ...
    summary = {"run_id": RUN_ID, "model_version": MODEL_VERSION, "symbol": SYMBOL, "final_bias": final_bias, "final_score": final_score, "confidence": min(abs(final_score) / 9, 1.0), "decision": "WAIT" if final_bias == "NEUTRAL" else f"{final_bias}_WATCH", "alignment": df["bias"].value_counts().to_dict()}
    report = os.path.join(LOGS_DIR, "audusd_forecast_report.csv")
    latest = os.path.join(LOGS_DIR, "audusd_forecast_latest.json")
