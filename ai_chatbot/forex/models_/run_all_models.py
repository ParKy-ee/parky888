...
MODEL_FILES = [
    ("AUDCAD", "generic_forex_model.py"),
    ("AUDCHF", "generic_forex_model.py"),
    ("AUDJPY", "generic_forex_model.py"),
    ("AUDNZD", "generic_forex_model.py"),
    ("AUDUSD", "generic_forex_model.py"),
    ("CADCHF", "generic_forex_model.py"),
    ("CADJPY", "generic_forex_model.py"),
    ("CHFJPY", "generic_forex_model.py"),
    ("EURAUD", "generic_forex_model.py"),
    ("EURCAD", "generic_forex_model.py"),
    ("EURCHF", "generic_forex_model.py"),
    ("EURGBP", "generic_forex_model.py"),
    ("EURJPY", "generic_forex_model.py"),
    ("EURNZD", "generic_forex_model.py"),
    ("EURUSD", "generic_forex_model.py"),
    ("GBPAUD", "generic_forex_model.py"),
    ("GBPCAD", "generic_forex_model.py"),
    ("GBPCHF", "generic_forex_model.py"),
    ("GBPJPY", "generic_forex_model.py"),
    ("GBPNZD", "generic_forex_model.py"),
    ("GBPUSD", "generic_forex_model.py"),
    ("NZDCAD", "generic_forex_model.py"),
    ("NZDCHF", "generic_forex_model.py"),
    ("NZDJPY", "generic_forex_model.py"),
    ("NZDUSD", "generic_forex_model.py"),
    ("USDCAD", "generic_forex_model.py"),
    ("USDCHF", "generic_forex_model.py"),
    ("USDJPY", "generic_forex_model.py"),
]
...
HUMAN_LOG_PATH = LOGS_DIR / "forex_models_human_log.md"
SUMMARY_CSV_PATH = LOGS_DIR / "forex_models_summary.csv"
...
def summarize_latest(symbol, payload, run_status):
    ...
    best = None
    if timeframes:
        best = max(timeframes, key=lambda row: abs(float(row.get("score", 0))))
    ...
    entry_price = ""
    entry_zone = "WAIT"
    if best and "WAIT" not in decision:
        current_price = best.get("price")
        support = first_value(best, ["support", "support_20", "support_80"])
        resistance = first_value(best, ["resistance", "resistance_20", "resistance_80"])
    ...
    return {
        ...
        "model_profile": summary.get("profile_name", ""),
        "watch_threshold": short_float(summary.get("watch_threshold"), 2),
        "trade_ready_threshold": short_float(summary.get("trade_ready_threshold"), 2),
        "reason": " | ".join(reasons),
        "returncode": run_status["returncode"],
    }
...
def write_human_log(rows, run_results):
    ...
    lines = [
        "# Forex Model Forecast Human Log",
        "",
        f"- Generated at: {now}",
...
def main():
    ...
    print("Running all forex forecast models...")
