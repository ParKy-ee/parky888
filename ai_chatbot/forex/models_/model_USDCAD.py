...
MODEL_VERSION = "USDCAD-forecast-v0.20-range-trend-hybrid"
...
def build_features(df, interval):
    ...
    df["support"] = df["low"].rolling(70).min()
    df["resistance"] = df["high"].rolling(70).max()
...
def score_row(row):
    score, reasons = 0, []
    trend_mode = row["adx"] >= 20 and row["atr_ratio"] >= 0.9
    if trend_mode:
        reasons.append("trend_mode")
        if row["close"] > row["ema200"] and row["ema20"] > row["ema100"] and row["roc"] > 0:
            score += 4; reasons.append("cad_trend_bull")
        elif row["close"] < row["ema200"] and row["ema20"] < row["ema100"] and row["roc"] < 0:
            score -= 4; reasons.append("cad_trend_bear")
    else:
        reasons.append("range_mode")
        if row["bb_pos"] <= 0.20 and row["rsi"] <= 38:
            score += 3; reasons.append("range_lower_reversion")
        elif row["bb_pos"] >= 0.80 and row["rsi"] >= 62:
            score -= 3; reasons.append("range_upper_reversion")
    ...
    if row["atr_ratio"] < 0.55:
        score = int(score * 0.6); reasons.append("dead_volatility_discount")
...
def forecast_interval(interval):
    ...
    action = "WAIT" if bias == "NEUTRAL" else f"{bias}_HYBRID_WATCH"
    return {
        ...
        "take_profit_1": row["close"] + 1.7 * row["atr"] if bias == "BULLISH" else row["close"] - 1.7 * row["atr"] if bias == "BEARISH" else np.nan,
...
def main():
    ...
    report = os.path.join(LOGS_DIR, "usdcad_forecast_report.csv")
    latest = os.path.join(LOGS_DIR, "usdcad_forecast_latest.json")
