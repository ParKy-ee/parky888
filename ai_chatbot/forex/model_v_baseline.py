...
AUDIT_CONSOLE_VERBOSE = CONSOLE_VERBOSE and not HUMAN_LOG_SCORE_ONLY
...
def print_backtest_summary(label, pnl_list, max_dd, final_eq, starting_equity, trades_df):
    ...
    if AUDIT_CONSOLE_VERBOSE and trades_df is not None and not trades_df.empty:
...
def print_key_backtest_summary(summary_rows):
    ...
    score_df = summary_df[
        summary_df["walk_forward_set"].isin(["Valid 2024", "OOS 2025+"])
    ].copy()
    if score_df.empty:
        print("No model score summary available.")
        return

    score_df = score_df.sort_values(["symbol", "interval", "walk_forward_set"])
    display_cols = [
        "symbol", "interval", "walk_forward_set", "trades", "win_rate",
        "return", "max_dd", "profit_factor", "expectancy", "sl_rate",
        "max_consecutive_loss",
    ]
    print("\nMODEL RUN SCORE SUMMARY")
    print(score_df[display_cols].to_string(index=False, float_format=lambda x: f"{x:0.4f}"))
