...
def fetch_yfinance_intraday(symbol, interval):
    ...
    request_interval = "1h" if interval.lower() == "4h" else interval
...
def add_higher_timeframe_trend(df, interval):
    ...
    if len(htf) < 220 and htf_rule != "1d":
    ...
    ema_fast_window = 20 if htf_rule == "1d" else 50
    ema_slow_window = 50 if htf_rule == "1d" else 200
    htf["htf_ema50"] = ta.trend.ema_indicator(htf["close"], window=ema_fast_window)
    htf["htf_ema200"] = ta.trend.ema_indicator(htf["close"], window=ema_slow_window)
...
def build_features(df, symbol, news_df, interval):
    ...
    df = add_higher_timeframe_trend(df, interval)
...
def main():
    ...
    for symbol in DAYTRADE_SYMBOLS:
        for interval in DAYTRADE_INTERVALS:
            ...
            feature_df = build_features(df, symbol, news_df, interval)
