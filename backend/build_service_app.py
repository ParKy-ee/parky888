...
MODELS_SRC = os.path.join(BASE_DIR, "models", "dynamic_trailing_model.joblib")
SERVICE_MODELS_DEST = os.path.join(BASE_DIR, "service", "models", "dynamic_trailing_model.joblib")

if os.path.exists(MODELS_SRC):
    shutil.copy(MODELS_SRC, SERVICE_MODELS_DEST)
    print(f"[+] คัดลอกโมเดลไปยัง: {SERVICE_MODELS_DEST}")
...
class AITradingDaemon:
    ...
    def evaluate_signals(self, processed_data):
        ...
        for symbol, (row, full_df) in processed_data.items():
            ...
            # ตรวจสอบว่าเข้าเงื่อนไขสัญญาณซื้อหรือไม่
            if confidence >= CONFIDENCE_THRESHOLD and row["market_bullish"] and row["rs_20d"] > 0:
                # ตรวจสอบว่าเคยส่งสัญญาณของวันนี้ไปแล้วหรือยัง
...
with open(os.path.join(BASE_DIR, "service", "app.py"), "w", encoding="utf-8") as f:
    f.write(app_code)

readme_code = """# 🐳 AI Swing Trading Service (Dockerized Background Daemon)
...
with open(os.path.join(BASE_DIR, "service", "README.md"), "w", encoding="utf-8") as f:
    f.write(readme_code)

print("[+] สร้างไฟล์ backend/service/app.py และ README.md สำเร็จ!")
